...
function enter() {
  ...
  if (mysqli_num_rows($r) == 1) {
    ...
    if (md5(md5($password).$w['u_salt']) == $w['u_pass']) {
      session_start();
      $uid = $w['u_id'];
      SetCookie("flexichess_login", $w['u_login'], time() + 50000, '/');
      SetCookie("flexichess_pass", md5($w['u_login'].$w['u_pass']), time() + 50000, '/');
      $_SESSION['flexichess_u_id'] = $uid;
      lastAct();
      return 1;
    }
    else {
...
function logout () {
  session_start();
  //echo "Current session u_id is ".$_SESSION['flexichess_u_id'];
  unset($_SESSION['flexichess_u_id']);
  SetCookie("flexichess_login", "", -1, '/');
  SetCookie("flexichess_pass", "", -1, '/');
  //echo "Current cookies: ".$_COOKIE['flexichess_login']." ".$_COOKIE['flexichess_pass'];
...
function login () {
  ...
  session_start();
  if (isset($_SESSION['flexichess_u_id'])) {
    if(isset($_COOKIE['flexichess_login']) && isset($_COOKIE['flexichess_pass'])) {
      SetCookie("flexichess_login", "", -1, '/');
      SetCookie("flexichess_pass","", -1, '/');
      SetCookie("flexichess_login", $_COOKIE['flexichess_login'], time() + 50000, '/');
      SetCookie("flexichess_pass", $_COOKIE['flexichess_pass'], time() + 50000, '/');
      $uid = $_SESSION['flexichess_u_id'];
      lastAct();
      load_user();
      return 1;
    }
    else {
      $r = mysqli_query($ml,"SELECT * FROM users WHERE u_id='{$_SESSION['flexichess_u_id']}'");
      echo mysqli_error($ml);
      if (mysqli_num_rows($r) == 1) {
        $w = mysqli_fetch_assoc($r);
        SetCookie("flexichess_login", $w['u_login'], time() + 50000, '/');
        SetCookie("flexichess_pass", md5($w['u_pass'].$w['u_pass']), time() + 50000, '/');
        $uid = $_SESSION['flexichess_u_id'];
        lastAct();
        load_user();
        return 1;
    ...
    if(isset($_COOKIE['flexichess_login']) && isset($_COOKIE['flexichess_pass'])) {
      $r = mysqli_query($ml,"SELECT * FROM users WHERE u_login='{$_COOKIE['flexichess_login']}'");
      echo mysqli_error($ml);
      $w = mysqli_fetch_assoc($r);
      if(mysqli_num_rows($r) == 1 && md5($w['u_login'].$w['u_pass']) == $_COOKIE['flexichess_pass']) {
        $uid = $w['u_id'];
        $_SESSION['flexichess_u_id'] = $uid;
        $uid = $_SESSION['flexichess_u_id'];
        lastAct();
        ...
        SetCookie("flexichess_login", "", time() - 360000, '/');
        SetCookie("flexichess_pass", "", time() - 360000, '/');
        return 0;
      }
    }
    else {
